...
A module (`app/Modules/<Module>/`) contains: `Controllers/`, `Requests/`, `Services/` (+ `Services/Contracts/`), `DTOs/`, `Mappers/`, `Models/`, `Providers/`, `Routes/api.php`, and (where it has tables) `Database/Migrations/` + `Database/Factories/`. Modules: `Shared`, `Auth`, `Category`, `Product`, `Stock`, `Dashboard`, `Shopify`, `Forecast`, `Intelligence`.
